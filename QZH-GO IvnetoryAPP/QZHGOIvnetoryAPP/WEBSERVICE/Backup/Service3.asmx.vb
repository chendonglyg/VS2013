...
Public Class Service3
    ...
    Public Function addcanshu(ByVal gqbh As String) As DataSet
        ...
        objdataadapter.SelectCommand = New SqlCommand
        objdataadapter.SelectCommand.Connection = objconnection
        objdataadapter.SelectCommand.CommandText = "SELECT midu.物料编码,midu_wuliaobianma.标准密度,midu.油品名称 FROM midu left outer join midu_wuliaobianma on midu.物料编码=midu_wuliaobianma.物料编码 where rtrim(ltrim(罐号名称))='" & Trim(gqbh) & "'"
 ...
 Public Function addkcload() As DataSet
    ...
    'y---------------------以下三个函数用于粮油称重系统收货安卓平台

    <WebMethod()> _
    Public Function for_sh_OK(ByVal forid As Integer, ByVal forusername As String, ByVal forstation As String) As Boolean '用于收货读取5个关键字段

        Dim objcommand As New SqlCommand

        objcommand.Connection = objconnection
        objcommand.CommandText = "update trade set 收货点=@station,库场收货人=@man,storehouse1=@storehouse1,storehouse1time=@storehouse1time where id =" & forid
        ...
        objcommand.Parameters.AddWithValue("@man", forusername)

        objcommand.Parameters.AddWithValue("@station", forstation)
    ...
    <WebMethod()> _
    Public Function for_sh_read() As DataTable
        Dim objconnection As New SqlConnection("Data Source= 10.229.17.6;Initial Catalog = toledo;User ID=sa; Password=chen dong")  '----------------小包装的仓区和货位以及模糊查询
        ...
        sqlobjdataadapter.SelectCommand.CommandText = "select id,truckno,product,ticketno,grossdatetime from trade where storehouse1=0"
        ...
        Return objdataset.Tables("lyg")
    End Function

    <WebMethod()> _
    Public Function for_sh_login(ByVal forusername As String, ByVal forpassword As String) As DataTable
        Dim objconnection As New SqlConnection("Data Source= 10.229.17.6;Initial Catalog = toledo;User ID=sa; Password=chen dong")  '----------------小包装的仓区和货位以及模糊查询

        objdataadapter.SelectCommand = New SqlCommand
        objdataadapter.SelectCommand.Connection = objconnection
        objdataadapter.SelectCommand.CommandText = "select * from users where username ='" & Trim(forusername) & "' and password ='" & Trim(forpassword) & "'"
        ...
        Return objdataset.Tables("users")
    End Function


    '----------------以下三个函数用于小包装称重系统收货安卓平台

    <WebMethod()> _
    Public Function for_xbzsh_OK(ByVal forid As Integer, ByVal forusername As String, ByVal forstation As String) As Boolean

        Dim objcommand As New SqlCommand
        Dim objconnection_xbz As New SqlConnection("Data Source= 10.229.196.254;Initial Catalog = toledo;User ID=sa; Password=chen dong")  '----------------小包装的仓区和货位以及模糊查询

        objcommand.Connection = objconnection_xbz
        objcommand.CommandText = "update trade set 收货点=@station,库场收货人=@man,storehouse1=@storehouse1,storehouse1time=@storehouse1time where id =" & forid
        ...
        If objconnection_xbz.State = ConnectionState.Closed Then
            objconnection_xbz.Open()
        End If
    ...
    <WebMethod()> _
    Public Function for_xbzsh_read() As DataTable
        Dim objconnection As New SqlConnection("Data Source= 10.229.196.254;Initial Catalog = toledo;User ID=sa; Password=chen dong")  '----------------小包装的仓区和货位以及模糊查询
    ...
    <WebMethod()> _
    Public Function for_xbzsh_login(ByVal forusername As String, ByVal forpassword As String) As DataTable
        Dim objconnection As New SqlConnection("Data Source= 10.229.17.6;Initial Catalog = toledo;User ID=sa; Password=chen dong")
