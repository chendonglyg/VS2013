...
Public Class Calculate
    ...
    Private Sub Calculate_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If sel_yewei.Checked = True Then
            guancunjieguo = MyService.just_Calculate(Val(gd2.Text), Val(gd3.Text), Val(yw2.Text), Val(yw3.Text), Trim(yp.Text), Trim(wlbm.Text), Trim(username), Trim(username), Trim(ssgq.Text), Trim(bzxx.Text), Trim(ygbh.Text), Val(gd.Text), Val(yw.Text), Val(qw.Text), Val(Trim(bzmd.Text)), Val(bzwd.Text), Val(bhxs.Text))
        ElseIf sel_chidu.Checked = True Then
            guancunjieguo = MyService.just_Calculate(Val(gg.Text) - Val(cd3.Text) + Val(cd4.Text), Val(gg.Text) - Val(cd5.Text) + Val(cd6.Text), Val(yw2.Text), Val(yw3.Text), Trim(yp.Text), Trim(wlbm.Text), Trim(username), Trim(username), Trim(ssgq.Text), Trim(bzxx.Text), Trim(ygbh.Text), Val(gg.Text) - Val(cd1.Text) + Val(cd2.Text), Val(yw.Text), Val(qw.Text), Val(Trim(bzmd.Text)), Val(bzwd.Text), Val(bhxs.Text))
        Else
            MessageBox.Show("必须选择并填写一种打尺高度参数", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If


        MessageBox.Show("货重:" & Math.Round(guancunjieguo, 3) & "吨", "计算结果", MessageBoxButtons.OK, MessageBoxIcon.Information)
