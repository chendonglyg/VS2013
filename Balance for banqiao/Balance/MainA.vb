...
#End Region
        ...
        If Standard.Checked = True Then '有两种情况，一种车号来自皮重库，一种来自配对模式的主库，来自皮重的不互换重量，来自配对主库的则按照配对模式来过磅
            ...
            Else
                ...
                Else
                    ...
                    If Val(maozhong.Text) < tempweight Then   '毛重小，是来装货的
    ...
    Private Sub savemove()
        ...
        If Trim(huowuliuxiang.Text) = "移库发货" Then
            ...
            If objdataview.Count = 0 Then '要创建，在前部分
                ...
                objcommandmove.CommandText = "insert into trade (mark,operator_tare_1,station_tare_1,truckno,contractno,product,specification,receiver,sender,shipname,transporter,pickupno,berth,amount,balanceno,ticketno,operator_gross_1,station_gross_1,gross,grossdatetime,tare,taredatetime,net,motorcade,motormen,done,finaldatetime) values (@mark, @operator_tare_1,@operator_tare_1,@truckno,@contractno,@product,@specification,@receiver,@sender,@shipname,@transporter,@pickupno,@berth,@amount,@balanceno,@ticketno,@operator_gross_1,@station_gross_1,@gross,@grossdatetime,@tare,@taredatetime,@net,@motorcade,@motormen,@done,@finaldatetime)"
                ...
                objcommandmove.Parameters.AddWithValue("@mark", "板桥")
            ...
            Else '要更新，也在前部分
                objcommandmove.Connection = objconnectionmove
                objcommandmove.CommandText = "update trade set mark=@mark,operator_gross_1=@operator_gross_1,operator_tare_1=@operator_tare_1,station_gross_1=@station_gross_1,station_tare_1=@station_tare_1,gross=@gross,grossdatetime=@grossdatetime,tare=@tare,taredatetime=@taredatetime,net=@net,done=@done where truckno='" & Trim(chehao.Text) & "' and ticketno='" & Trim(piaohao.Text) & "'"
                ...
                objcommandmove.Parameters.AddWithValue("@gross", Trim(maozhong.Text))
                objcommandmove.Parameters.AddWithValue("@grossdatetime", Trim(maozhongshijian.Text))
                objcommandmove.Parameters.AddWithValue("@tare", Trim(pizhong.Text))
                objcommandmove.Parameters.AddWithValue("@taredatetime", Trim(pizhongshijian.Text))
                objcommandmove.Parameters.AddWithValue("@net", Trim(jingzhong.Text))
                objcommandmove.Parameters.AddWithValue("@finaldatetime", Now)
                objcommandmove.Parameters.AddWithValue("@done", Val(1))
                objcommandmove.Parameters.AddWithValue("@mark", "板桥")
        ...
        If Trim(huowuliuxiang.Text) = "移库收货" Then
            ...
            Else '更新且后半部分
                ...
                objcommandmove.Parameters.AddWithValue("@gross2", Trim(maozhong.Text))
                objcommandmove.Parameters.AddWithValue("@grossdatetime2", Trim(maozhongshijian.Text))
                objcommandmove.Parameters.AddWithValue("@tare2", Trim(pizhong.Text))
                objcommandmove.Parameters.AddWithValue("@taredatetime2", Trim(pizhongshijian.Text))
                objcommandmove.Parameters.AddWithValue("@net2", Trim(jingzhong.Text))
                objcommandmove.Parameters.AddWithValue("@finaldatetime2", Now)
                objcommandmove.Parameters.AddWithValue("@mark", "板桥")
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        If Standard.Checked = True Then
            ...
            If zhupibiaozhi = True Then '皮重库确实有，那么一次性保存完毕
                ...
                objcommand.Parameters.AddWithValue("@finaldatetime", outtime)
            ...
            Else
                ...
                If (biaozhi = 0) Then   '皮重库没有皮重，只能做第一次过磅
                    Dim objcommand As New SqlCommand
                ...
                If biaozhi Then     '第二次过磅


                    If storehouse1.Text = "Ture" Then
                        Dim objcommand As New SqlCommand
        ...
        If Tare.Checked = True Then
            ...
            If objdataview.Count = 0 Then
                ...
                objcommand.Parameters.AddWithValue("@truckno", Trim(chehao.Text))
                objcommand.Parameters.AddWithValue("@tare", Val(pizhong.Text))
                objcommand.Parameters.AddWithValue("@outtime", intime)
        ...
        If Manual.Checked = True Then
            ...
            objcommand.Parameters.AddWithValue("@grossdatetime", intime)  '这里用intime还是outime都无所谓，因为手工制单一次通过

            objcommand.Parameters.AddWithValue("@tare", Val(pizhong.Text))
            objcommand.Parameters.AddWithValue("@taredatetime", intime) '这里用intime还是outime都无所谓，因为手工制单一次通过
            objcommand.Parameters.AddWithValue("@net", Val(jingzhong.Text))
    ...
    Private Sub chehao_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chehao.SelectedIndexChanged
