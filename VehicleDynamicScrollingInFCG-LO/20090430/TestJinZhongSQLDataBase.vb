Public Class TestJinZhongSQLDataBase
    ...
    Private Sub Total_Record_Iscomplete()
        ...
        objdataadapteriscomplete.SelectCommand.CommandText = "select Top 50 TruckNO as 车号,MaterialName as 物料,PlanWeigh as 计划重量,CreateTime as 制卡时间,FirstWeightTime as 一磅时间,SecondWeightTime as 二磅时间,MaterialName as 发货用时,CheckOutTime as 收卡时间, FinalNet as 净重 from TaskInfoForOutSysCLDT where iscomplete=1"
        ...
        For a = 0 To objdataviewiscomplete.Count - 1
            ...
            b = Convert.ToDateTime(objdatasetiscomplete.Tables("iscomplete").Rows(a).Item(4).ToString)

            c = Convert.ToDateTime(objdatasetiscomplete.Tables("iscomplete").Rows(a).Item(5).ToString)
            'MessageBox.Show(b & c)
    ...
    Private Sub Total_record()
        Try
            ...
            objdataadapter.SelectCommand.CommandText = "select TruckNO as 车号,MaterialName as 物料,PlanWeigh as 计划重量,CreateTime as 发卡时间,isfirstweight as 完成一磅,BeginTallyTime as 开始理货,EndTallyTime as 结束理货,IssecondWeight as 完成二磅 from TaskInfoForOutSysCLDT where iscomplete=0 "
    ...
    Private Sub dgv_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgv.CellFormatting
        ...
        If RowCount > 1 Then
            ...
            Try
                ...
                If dgv.Rows(e.RowIndex).Cells("开始理货").Value <> "" Then
                    dgv.Rows(e.RowIndex).Cells("开始理货").Style.BackColor = Color.DarkGreen

                End If

                If dgv.Rows(e.RowIndex).Cells("结束理货").Value <> "" Then
